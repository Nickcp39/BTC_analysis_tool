"""
StepA2: 2025 峰前到 08-15，峰后从 10-03 开始，中间 08-16~10-02 跳过
- 峰前：减半→08-15，锚点 08-15
- 峰后：10-03→最新，dd_pct 仍相对 08-15 价；中间日期不画，图上会有断点
- 2017/2021 无断点，与 A1 一致

TODO/FIXME: 此处有问题待修复（2025 峰前峰后逻辑、2017/2021 数据对齐等）
"""
...
# 2025 特殊：峰前到 08-15，峰后从 10-03 开始
...
def main():
    ...
    # 2025：峰前 减半→08-15，峰后 10-03→最新，中间跳过
    # 峰前：窗口到 08-15
    ...
    # 峰后：从 10-03 到最新，dd_pct 仍相对 08-15 价
    ...
    # 2025 峰后 x：从 1 开始（0=08-15 峰顶），1=10-03，体现中间跳过
    x25_post = np.linspace(1, N_ref, len(c25_post)) if len(c25_post) > 1 else np.array([1.0])

    # ----- 画图：2025 峰前+断点+峰后，2017/2021 连续 -----
    COLORS = {"2025": "#1a5276", "2021": "#e74c3c", "2017": "#27ae60"}  # 深蓝、红、绿
    plt.figure(figsize=(13, 5.5))

    # 2025：峰前到 0，峰后从 x=1 开始（中间 08-16~10-02 不画，线会断开）
    ...
    plt.plot(x21_full, y21_full, color=COLORS["2021"], label=f"2021（峰前×{scale21:.2f} 峰后原幅）", zorder=2)

    x17_full = np.concatenate([pre17.index.values, x17])
    y17_full = np.concatenate([pre17.values, p17_post.values])
    plt.plot(x17_full, y17_full, color=COLORS["2017"], label=f"2017（峰前×{scale17:.2f} 峰后原幅）", zorder=2)

    plt.axvline(0, linestyle="--", color="gray", linewidth=1.2, zorder=1)
    plt.ylabel("相对峰顶涨跌（%）")
    plt.xlabel("左：峰前相对天数（锚点=08-15） | 右：峰后天数（2025 从 10-03 起，中间跳过）")
    plt.title(f"StepA2：2025 峰前到 08-15、峰后从 10-03 开始，08-16~10-02 跳过 | 最新={latest_25.strftime('%Y-%m-%d')}")
    plt.legend(loc="lower left")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    out_png = PNG_DIR / "stepA2_peak_skip_gap.png"
    plt.savefig(out_png, dpi=170)
    plt.close()

    lines = [
        "=== StepA2 2025 峰前 08-15 峰后 10-03 中间跳过 ===",
        f"2025 峰前锚点: {PEAK_2025_PRE.strftime('%Y-%m-%d')}",
        f"2025 峰后起点: {PEAK_2025_POST_START.strftime('%Y-%m-%d')}",
        f"跳过: 08-16 ~ 10-02",
        f"峰后天数: 2017={days_17}, 2021={days_21}, 2025={days_25}",
    ]
    with open(OUTDIR / "stepA2_notes.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))

    print("StepA2 done. 2025: pre to 0815, post from 1003, skip 0816-1002")
