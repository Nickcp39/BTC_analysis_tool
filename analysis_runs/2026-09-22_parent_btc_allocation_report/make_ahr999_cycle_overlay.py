...
LATEST = pd.read_csv(PRICE_CSV, usecols=["date"], parse_dates=["date"])["date"].max()  # 数据最新日（2026-09-22 版起不再手写）
...
def make_frame() -> pd.DataFrame:
    ...
    ahr.to_csv(REPORT_DIR / f"ahr999_recalc_to_{LATEST.date().isoformat()}.csv", index=False)
...
def main() -> None:
    ...
    for label, peak, color, lw, zorder, alpha, is_hero in PEAKS:
        ...
        if is_hero:
            ...
            # 2026-09-22 版：两轮低点只差 2 天，标签分上下两行放在左下，避免重叠
            label_y = 0.17 if label.startswith("2017") else 0.08
            ax.annotate(
                f"{label[:4]} 最低 {min_row['ahr999']:.3f}（第 {min_day} 天）",
                xy=(min_day, min_row["ahr999"]),
                xytext=(min_day - 128, label_y),
    ...
    # 2026-09-22 版新增：前两轮 AHR 低点都在顶后第 364/366 天 —— 标出这条「365 天」参照带
    ax.axvspan(364, 366, color="#475569", alpha=0.18, zorder=1)
    ax.axvline(365, color="#475569", ls=":", lw=1.2, zorder=1)
    ax.text(369, 1.28, "前两轮低点\n第 364 / 366 天", fontsize=8.5, color="#475569", va="top")
    # 本轮第 365 天前后：8 月中那个「更高的低点」（没破 6 月低点，随后起飞）
    hero_peak = PEAKS[-1][1]
    near = ahr[(ahr["date"] >= hero_peak + pd.Timedelta(days=350)) & (ahr["date"] <= hero_peak + pd.Timedelta(days=375))]
    if len(near):
        hl = near.loc[near["ahr999"].idxmin()]
        hl_day = int((hl["date"] - hero_peak).days)
        ax.scatter([hl_day], [hl["ahr999"]], s=55, color="#0f766e", zorder=8, edgecolor="white", linewidth=1.2)
        ax.annotate(
            f"{hl['date'].date().isoformat()} 第 {hl_day} 天\n更高的低点 AHR {hl['ahr999']:.3f}\n（没破 6 月低点，随后起飞）",
            xy=(hl_day, hl["ahr999"]), xytext=(hl_day + 22, hl["ahr999"] - 0.24),
            fontsize=8.6, color="#0f766e",
