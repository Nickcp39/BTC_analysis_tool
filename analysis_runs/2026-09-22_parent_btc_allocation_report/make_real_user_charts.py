...
def plot_lth_panel(ax: plt.Axes, lth: pd.DataFrame) -> tuple[float, float]:
    ...
    # 2026-09-22 版：画出全部公开基准点（不再只画首尾两点），8 月的回落要能看见
    ax.set_title("低频资本：LTH 供应（公开基准点，非连续曲线）", fontsize=12.5, fontweight="bold")
    green, red, gray = "#0f766e", "#b91c1c", "#64748b"
    xs = list(range(len(lth)))
    vals = (lth["lth_supply_btc"] / 1e6).tolist()
    ax.plot(xs, vals, color=gray, lw=1.6, ls="--", zorder=2)
    for i, row in lth.iterrows():
        color = green if row["phase"] == "accumulation" else red
        ax.scatter([i], [vals[i]], s=90, color=color, zorder=3)
        ax.text(i, vals[i] + 0.045,
                f"{row['date'].date()}\n{vals[i]:.2f}M（{row['lth_ratio']:.1%}）\n{'吸筹' if row['phase'] == 'accumulation' else '派发'}",
                ha="center", fontsize=9.2, color=color, fontweight="bold")
    ax.set_xlim(-0.5, len(lth) - 0.5)
    ax.set_ylim(min(vals) - 0.25, max(vals) + 0.28)
    ax.set_xticks([])
    ax.set_ylabel("百万 BTC")
    ax.grid(axis="y", alpha=0.2)
    ax.text(0.5, -0.1,
            "8 月读数一部分是托管迁移（不全是卖出）；CryptoQuant 30 天 LTH 变化：08-30 -10.6 万 → 09-20 -2.2 万（仍为负、在收窄）",
            transform=ax.transAxes, ha="center", fontsize=8.6, color="#334155")
...
def main() -> None:
    ...
    # 2026-09-22 版：用报告目录内的 LTH 基准点（在 data/lth_metrics.csv 基础上加了 08-07 一行）
    lth = pd.read_csv(OUT_DIR / "lth_metrics_2026-09-22.csv", parse_dates=["date"])
